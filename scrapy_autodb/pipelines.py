...
class AutoDBPipeline(object):
    ...
    def _connect_db(self):
        ...
        self.db_pool = adbapi.ConnectionPool(adapter, **db_params, cp_reconnect=True)
    ...
    def open_spider(self, spider):
        ...
        for t_item in self.l_items:
            ...
            # model._meta.add_field("airflow_modify_date", peewee.DateTimeField(default=datetime.now))
    ...
    def _insert_db(self, cursor, item):
        ...
        if model.__name__ == "CompanyInfo":
            d["airflow_modify_date"] = datetime.now()
        with self.db:
            if db_engine.lower() == "mysql":
                sql = model.insert(**d).on_conflict_replace().sql()
            elif db_engine.lower() == "postgresql":
